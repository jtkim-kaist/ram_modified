...
def calc_reward(outputs):
    ...
    J = tf.concat(axis=1, values=[tf.log(p_y + SMALL_NUM) * onehot_labels_placeholder, tf.log(p_loc_first + SMALL_NUM) * (R - no_grad_b)])
    # J = tf.concat(axis=1, values=[tf.log(p_y + SMALL_NUM) * (onehot_labels_placeholder),
    #                               tf.log(p_loc_first + SMALL_NUM) * (R - b)])
    # J = tf.concat(axis=1, values=[tf.log(p_y + SMALL_NUM) * (onehot_labels_placeholder),
    #                                                              tf.log(p_loc_first + SMALL_NUM) * R])
